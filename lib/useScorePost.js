...
const getError = (totalScore, numberOfScores) => {
  let error = "Couldn't post: "
  if (numberOfScores === 9 && (totalScore < 27 || totalScore >= 90)) {
    error = error.concat('the value of totalScore must be between 27 and 90 ')
  }
  if (numberOfScores === 18 && (totalScore < 90 || totalScore >= 180)) {
    error = error.concat('the value of totalScore must be between 90 and 180 ')
  }
  return error
}
